...
pygame.display.set_caption('F-Bird')

from os.path import join 
pipe_img = pygame.transform.scale2x(pygame.image.load(join('imgs','pipe.png')).convert_alpha())
bg_img = pygame.transform.scale(pygame.image.load(join('imgs','bg.png')).convert_alpha(), (600, 900))
bird_images = [pygame.transform.scale2x(pygame.image.load(join('imgs','bird' + str(x) + '.png'))) for x in range(1,4)]
base_img = pygame.transform.scale2x(pygame.image.load(join('imgs','base.png')).convert_alpha())
...
class Bird:
	...
	def move(self):
		...
		# for downward acceleration, calculate displaccement
		...
		if displacement < 0 or self.pos_y < self.height + 50:	# tilt up
	...
	def draw(self, win):
		...
		# tilt the bird
		blitRotateCenter(win, self.img, (self.pos_x, self.pos_y), self.tilt)
...
class Pipe:
	...
	def __init__(self, pos_x):
		...
		self.width = self.TOP_PIPE.get_width()
		self.set_height()
	...
	def draw(self, win):
		...
		:return: None
		'''
		win.blit(self.TOP_PIPE, (self.pos_x, self.top))
		win.blit(self.BOTTOM_PIPE, (self.pos_x, self.bottom))
...
class Base:
	...
	def draw(self, win):
		...
		:return: None
		'''
		win.blit(self.BASE, (self.x1, self.pos_y))
		win.blit(self.BASE, (self.x2, self.pos_y))

	def collide(self, bird):
		if bird.pos_y + bird.img_height - 10 >= FLOOR or bird.pos_y < -50:
			return True
...
def run_game(genomes, config):
	...
	while True:
		...
		for pipe in pipes:
			...
			# Increase score once bird.pos_x passes pipes
